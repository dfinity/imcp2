...
| No catch-all read/write tool; reads and writes are separate tools | ⏳ 18 read-only tools once [#153](https://github.com/dfinity/imcp2/pull/153) lands (incl. the instructions-only top-up); writes split per operation |
...
>    "Questions only" exercises the 18 read-only-annotated tools; "Actions &
