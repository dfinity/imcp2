...
6. Add `icpBindgen()` to `vite.config.js` (see `skill://icp-cli/references/binding-generation.md`)
7. Replace actor setup code: use `safeGetCanisterEnv` from `@icp-sdk/core` + `createActor` from generated bindings (see `skill://icp-cli/references/binding-generation.md`)
